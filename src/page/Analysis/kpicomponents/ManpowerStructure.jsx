...
import { manpowerStructureColumns } from '../../../utils/tableTitle.config';
import './turnoverRate.less';

@connect(state => state.analysis, actionCreators)
class ManpowerStructure extends Component {
  constructor(props) {
    super(props);
    this.state = {
      dateString: ''
  ...
  handleSearchTurnoverRate = () => {
    ...
    const { queryKpiWeekEmployeeDataAnalysis } = this.props;
    queryKpiWeekEmployeeDataAnalysis(dateString);
  };
  render() {
    const { manpowerStructureList } = this.props;
    return (
        ...
        <Row>
          ...
          <Col className="turnover-rate-content-table" span={24}>
            <Table
              ...
              columns={manpowerStructureColumns}
              dataSource={manpowerStructureList}
              scroll={{ x: '100%' }}
              pagination={false}
            />
          </Col>
...
export default ManpowerStructure;
