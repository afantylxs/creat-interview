...
class WeeklyJobClass extends Component {
  constructor(props) {
    super(props);
    this.state = {
      endDate: '',
  ...
  handleSearchTurnoverRate = () => {
    ...
    const { queryEmployeeCountByJobClass } = this.props;
    queryEmployeeCountByJobClass({ currentPage: 1, endDate });
    this.setState({
      currentPage: 1
    });
  };
  ...
  handleTableChange = page => {
    ...
    const { queryEmployeeCountByJobClass } = this.props;
    queryEmployeeCountByJobClass(arg0);
    this.setState({
      currentPage: page
    });
  };
  render() {
    ...
    const { weeklyJobClassList, weeklyJobClassTotal } = this.props;
    return (
      <div className="turnover-rate">
        <Row>
          ...
          <Col className="general-paging" span={24}>
            <Pagination
              total={weeklyJobClassTotal}
              showTotal={weeklyJobClassTotal =>
                `共 ${weeklyJobClassTotal} 条数据`
              }
              current={currentPage}
              onChange={page => {
